...
describe('PhotoGallery component', () => {
  it('renders photos information', () => {
    ...
    photos.forEach((photo, index) => {
      expect(
        within(photoElements[index]).getByText(photo.description as string)
      ).toBeInTheDocument()
      expect(
        within(photoElements[index]).getByText(photo.user.name)
      ).toBeInTheDocument()
    })
  })
  ...
  it('calls onPageChange when pagination changes', () => {
    ...
    const secondPageButton = within(
      screen.getByTestId('photo-gallery--pagination')
    ).getByText(newPage)
